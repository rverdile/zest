# Go API client for pulpGoBinding
...
```golang
import pulpGoBinding "github.com/content-services/pulpGoBinding/packages/pulpGoBinding"
```
...
```golang
ctx := context.WithValue(context.Background(), pulpGoBinding.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), pulpGoBinding.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), pulpGoBinding.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), pulpGoBinding.ContextOperationServerVariables, map[string]map[string]string{
