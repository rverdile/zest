...
> AsyncOperationResponse ContentAnsibleCollectionVersionsCreate(ctx).Artifact(artifact).File(file).Repository(repository).Upload(upload).ExpectedName(expectedName).ExpectedNamespace(expectedNamespace).ExpectedVersion(expectedVersion).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.ContentCollectionVersionsApi.ContentAnsibleCollectionVersionsCreate(context.Background()).Artifact(artifact).File(file).Repository(repository).Upload(upload).ExpectedName(expectedName).ExpectedNamespace(expectedNamespace).ExpectedVersion(expectedVersion).Execute()
    if err != nil {
