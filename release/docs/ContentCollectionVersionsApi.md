...
> AsyncOperationResponse ContentAnsibleCollectionVersionsCreate(ctx).Upload(upload).Repository(repository).Artifact(artifact).File(file).ExpectedName(expectedName).ExpectedNamespace(expectedNamespace).ExpectedVersion(expectedVersion).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.ContentCollectionVersionsApi.ContentAnsibleCollectionVersionsCreate(context.Background()).Upload(upload).Repository(repository).Artifact(artifact).File(file).ExpectedName(expectedName).ExpectedNamespace(expectedNamespace).ExpectedVersion(expectedVersion).Execute()
    if err != nil {
