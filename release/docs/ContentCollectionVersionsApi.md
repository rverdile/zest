...
> AsyncOperationResponse ContentAnsibleCollectionVersionsCreate(ctx).Repository(repository).Artifact(artifact).File(file).Upload(upload).ExpectedName(expectedName).ExpectedNamespace(expectedNamespace).ExpectedVersion(expectedVersion).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.ContentCollectionVersionsApi.ContentAnsibleCollectionVersionsCreate(context.Background()).Repository(repository).Artifact(artifact).File(file).Upload(upload).ExpectedName(expectedName).ExpectedNamespace(expectedNamespace).ExpectedVersion(expectedVersion).Execute()
    if err != nil {
